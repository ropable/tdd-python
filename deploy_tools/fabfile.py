...
def _create_directory_structure_if_necessary(site_name):
    for subfolder in ('database', 'static', 'virtualenv', 'source', 'logs'):
        local('mkdir -p {0}/{1}/{2}'.format(SITES_FOLDER, site_name, subfolder))


def _get_latest_source(source_folder):
    if exists(source_folder + '/.git'):
        local('cd {0} && git fetch'.format(source_folder))
    else:
        local('git clone {0} {1}'.format(REPO_URL, source_folder))
    current_commit = local('git log -n 1 --format=%H', capture=True)
    local('cd {0} && git reset --hard {1}'.format(source_folder, current_commit))
...
def _update_virtualenv(source_folder):
    ...
    if not exists(virtualenv_folder + '/bin/pip'):
        local('virtualenv --python=python3.3 {0}'.format(virtualenv_folder))
    local('{0}/bin/pip install -r {1}/requirements.txt'.format(
        virtualenv_folder, source_folder))


def _update_static_files(source_folder):
    local('cd {0} && ../virtualenv/bin/python3 manage.py collectstatic --noinput'.format(
        source_folder))


def _update_database(source_folder):
    local('cd {0} && ../virtualenv/bin/python manage.py syncdb --noinput'.format(
        source_folder))
